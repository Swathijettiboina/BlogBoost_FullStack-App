        ...
        <div className="bg-gradient-to-r from-blue-50 via-white to-gray-100 min-h-screen py-10">
            <div className="max-w-5xl mx-auto p-6 bg-white shadow-2xl rounded-xl relative">
                ...
                <div className="absolute top-4 right-4 flex space-x-3">
                    <Link to={`/edit-blog/${id}`} className="bg-blue-500 text-white px-4 py-2 rounded-full flex items-center space-x-1 hover:bg-blue-600 transition">
                        <Edit size={16} />
                        <span>Edit</span>
                    </Link>
                    <button onClick={handleDelete} className="bg-red-500 text-white px-4 py-2 rounded-full flex items-center space-x-1 hover:bg-red-600 transition">
                        <Trash size={16} />
                        <span>Delete</span>
                    </button>
                </div>
                ...
                <div className="flex justify-between text-gray-600 text-sm mb-5">
                    ...
                    <p>Published on {blog.publishDate ? new Date(blog.publishDate).toLocaleDateString() : "Unknown Date"}</p>
                </div>

                {/* Blog Content */}
                <p className="text-gray-700 leading-relaxed text-lg mb-6">{blog.content}</p>
                ...
                <div className="flex items-center justify-between mt-6 text-gray-600">
                    ...
                    <button onClick={likeHandle} className='flex items-center space-x-2'>
                        <Heart size={16} className={`hover:cursor-pointer transition duration-300 ${liked ? 'text-red-500 fill-red-500' : 'text-gray-600'}`} />
